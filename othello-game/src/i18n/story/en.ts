/**
 * English story content — root composer.
 *
 * Source: `othello-game/docs/scenario/01_PLR00_main_story_part{1..4}.md`
 * Author: user (Pack 1, received 2026-05-04).
 *
 * v0.36.78 migrated to a **per-PLR co-located folder structure**. Each
 * chain-step PLR (PLR02..PLR20) lives in `./plr/PLR0M_<slug>/{ja,en}.ts`
 * as a PlrPackage; this file is now a thin orchestrator that imports
 * those packages and composes the `prologueByPlr` /
...
import { PLR02_MIKOTO_EN } from './plr/PLR02_mikoto/en';
import { PLR03_RIN_EN } from './plr/PLR03_rin/en';
import { PLR04_REN_EN } from './plr/PLR04_ren/en';

/** Ordered list of chain-step PLR packages for EN. */
const PLR_PACKAGES_EN: ReadonlyArray<PlrPackage> = [
  PLR02_MIKOTO_EN,
  PLR03_RIN_EN,
  PLR04_REN_EN,
];

const prologueByPlr: Record<number, PrologueContent> = {};
const chapterStoriesByPlr: Record<number, ReadonlyArray<ChapterStory>> = {};
const narrativeByPlr: Record<
  number,
  Partial<{
...
> = {};
const chainStepEndingByPlr: Record<number, NarrativeScene> = {};

for (const p of PLR_PACKAGES_EN) {
  prologueByPlr[p.plrIdx] = p.prologue;
  chapterStoriesByPlr[p.plrIdx] = p.chapters;
  if (p.narrative) narrativeByPlr[p.plrIdx] = p.narrative;
  if (p.chainStepEnding) chainStepEndingByPlr[p.plrIdx] = p.chainStepEnding;
}
