...
- **真エンディング (Phase 3 → Phase 4 Step 3 で 4 段化)**: PLR01
  英霊ハルキで章 20 をクリアすると以下が連鎖:
  1. `trueEndingAchieved` フラグ立ち (`localStorage('othello:true_ending_achieved') = '1'`) → 隠し Lv.21 OPP21 がアンロック
  2. シネマティック `narrative.trueEnding20B` (解放の瞬間) →
     `narrative.trueEnding20C` (現世エピローグ) → **`narrative.trueEnding20D` (Phase 4 Step 3 追加: ヴォイドφ覚醒)** が NarrativeOverlay 経由で連続再生
  3. 20-D dismiss 時に `voidphiAwakened` フラグ立ち
     (`localStorage('othello:voidphi_awakened') = '1'`) → 隠し Lv.22
     **OPP22 ヴォイドφがアンロック** (この時点で Lv.21 と Lv.22 が
     別フラグで個別管理される)
  4. 視聴済みは scene archive に登録、`getOrderedArchiveScenes` の
