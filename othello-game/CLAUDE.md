...
| 15 | DJ 風牙＆雷牙 | 双 | 神話の双子 DJ デュオ | `/avatars/opponents/OPP15_fuga_raiga/icon.png` | 最初から |
...
旧 256×256 簡易版は `archive/avatars-old/opponents/` に温存（`public/` の外＝ビルド対象外）。詳細は `public/avatars/README.md` および `public/avatars/opponents/INDEX.md` 参照。
...
ライトノベル主人公アーキタイプの portrait を `public/avatars/players/` に格納。デフォルト ("PLR00 / あなた") は index 0、続く 19 体は通常アンロック対象、最後の **PLR01 英霊ハルキは Phase 3 で 1024×1024 RGBA フォルダ構造に差し替え済み** (`PLR01_haruki_heroic/character.png` + `background.png` + `icon.png` + `spec.md`)。旧 PLR01 単体 PNG は `archive/avatars-old/players/PLR01_haruki.png` に温存。
...
`archive/avatars-old/players/PLRxx_<oldslug>.png` に履歴保存（`public/` の外）。
...
- **カタカナ** で外来・異質キャラ（メル・シキ・シオン・ルナ・アキラ・アリア・レオン・ゼロ）
- **複合表記**（プレフィックス＋全角＆連結）で双体・並立／ユニットキャラ（例: DJ 風牙＆雷牙）
