...
- PLR01 で章 20 クリア時 → 真エンディング達成 = `trueEndingAchieved` フラグ立ち = OPP21 が以後フリーモードで選択可能 + (将来) `OPP21_zero_unmasked/background.png` のフルスクリーン演出

実装は `App.tsx` 内の `aiAvatarImage` 派生 + `zeroAvatarFor()` ヘルパーで、`ZERO_HOODED = OPP20_zero/icon.png` と `ZERO_UNMASKED = OPP21_zero_unmasked/icon.png` を切り替える。アンロック判定 `trueEndingAchieved` は `localStorage('othello:true_ending_achieved')` に永続化される。各キャラの正の記述は `public/avatars/opponents/OPPxx_*/spec.md` 参照。
...
| index | 名前 | 主人公アーキタイプ | ファイル | アンロック |
|---|---|---|---|---|
| 0 | あなた (PLR00 default) | 盤上世界の旅人 | `players/PLR00_default.png` | 最初から |
| 1 | 美琴 | 魔法学園の天才 | `players/PLR02_mikoto.png` | 1 周クリア |
| 2 | リン | VRMMOの最強プレイヤー | `players/PLR03_rin.png` | 2 周 |
| 3 | 蓮 | 剣道部主将 | `players/PLR04_ren.png` | 3 周 |
| 4 | 千歳 | タイムリープ少女 | `players/PLR05_chitose.png` | 4 周 |
| 5 | 晴 | 現代の陰陽師 | `players/PLR06_haru.png` | 5 周 |
| 6 | カイ | 空の冒険者 | `players/PLR07_kai.png` | 6 周 |
| 7 | 千夏 | 聖剣の村娘 | `players/PLR08_chinatsu.png` | 7 周 |
| 8 | 透 | 学園名探偵 | `players/PLR09_toru.png` | 8 周 |
| 9 | ノア | 未来から来た少女 | `players/PLR10_noa.png` | 9 周 |
| 10 | 凪 | 異世界料理人 | `players/PLR11_nagi.png` | 10 周 |
| 11 | エル | 元魔王、今は転校生 | `players/PLR12_el.png` | 11 周 |
| 12 | スミレ | 記憶喪失の冒険者 | `players/PLR13_sumire.png` | 12 周 |
| 13 | 葉月 | 機械工学の天才 | `players/PLR14_hazuki.png` | 13 周 |
| 14 | 隼人 | 凄腕ガンナー | `players/PLR15_hayato.png` | 14 周 |
| 15 | ひかり | 光の精霊使い | `players/PLR16_hikari.png` | 15 周 |
| 16 | ヨル | 半吸血鬼 | `players/PLR17_yoru.png` | 16 周 |
| 17 | 湊 | 海の冒険者 | `players/PLR18_minato.png` | 17 周 |
| 18 | 奏太 | 天才ピアニスト | `players/PLR19_souta.png` | 18 周 |
| 19 | 悠 | 神話の英雄 | `players/PLR20_yu.png` | 19 周 |
| **🌟 20** | **英霊ハルキ (PLR01)** | **英霊化された旅人 (時間逆転ロア)** | `players/PLR01_haruki_heroic/icon.png` | **PLR00 で章 20 クリア後 (= 20 周目の特別枠)** |

**PLR01 の特殊性**: 未来の英霊化形態のハルキ。PLR00 で章 20 をクリアした時点でアンロックされ、AVATARS 配列の最後尾 (index 20) に並ぶ。PLR01 を選択して章 20 をクリアすると**真エンディング達成**となり `trueEndingAchieved` フラグが立ち、隠し OPP21 ゼロ (現世帰還) + OPP22 ヴォイドφ が同時にアンロックされる。

検出ロジック: `AVATARS[p1Avatar].image.includes('PLR01_haruki_heroic')` (App.tsx の gameOver 効果内 + `aiAvatarImage` 派生)。
...
- **真エンディング (Phase 3)**: PLR01 英霊ハルキで章 20 をクリアすると
  以下が連鎖:
  1. `trueEndingAchieved` フラグ立ち (`localStorage('othello:true_ending_achieved') = '1'`)
  2. 隠し Lv.21 OPP21 + Lv.22 OPP22 が選択画面でアンロック
     (両者とも `hidden: true` フラグ + `c.hidden && !trueEndingAchieved`
     ガードで `???` 表示されていたものが解放)
  3. シネマティック `narrative.trueEnding20B` (解放の瞬間) →
     `narrative.trueEnding20C` (現世エピローグ) が NarrativeOverlay
     経由で連続再生 → 視聴済みフラグはシーン回想に登録
  実装は App.tsx gameOver 効果内 + storyOverlay 連鎖 dismiss。
  挿絵は `/illustrations/trueEnding20{B,C}-{landscape,portrait}.png`
  (NarrativeOverlay の `imageBaseName` に渡される)。
