...
> 目標: **9 シーン** (intro chain 5 + mid-route 4) × landscape/portrait = **18 枚**の挿絵を生成・パッケージング (v0.36.56 で intro chain を per-PLR 化、5 シーン追加)
...
中身 (約 4.6MB / 10 ファイル) — 9 シーン仕様 (v0.36.56):
```
...
> **重要**: 9 シーン全部の英文 ChatGPT プロンプトは **`02_SCENE_spec_4_scenes.md`** に記載されています (ファイル名は歴史的経緯で `4_scenes` だが中身は 9 シーン版に更新済)。以下では mid-route 4 シーンの詳細だけ抜粋して再掲、intro chain 5 シーンは spec.md 側を参照してください。

### intro chain 5 シーンのサマリ (詳細プロンプトは spec.md §1-5 参照)
...
### 4.1 シーン 6/9: solitude — 大聖堂図書館の静夜
...
合格した 18 枚 (9 シーン × landscape/portrait) を以下のいずれかで Claude Code 側に渡してください:
...
18 枚すべてを同時に揃える必要はありません。**準備できたシーンから順次納品**してください。Code 側は**二段 fallback**を実装しているため、未生成のシーンは自動的に共通版 (`_shared/<scene>-<orientation>.png`) で埋まります。テキストは美琴版のまま。9 シーン全部・5 シーン (intro chain) のみ・4 シーン (mid-route) のみ、いずれの粒度の納品も OK。
