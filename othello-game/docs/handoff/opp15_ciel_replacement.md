...
- **旧アセットは `archive/avatars-old/` へ退避**（削除しない）。
...
| `archive/avatars-old/opponents/OPP15_ciel.png` | 旧 256px 簡易版（**触らない・温存**） | 256×256 PNG |
...
2. **旧アセット退避**:
   - `mkdir -p archive/avatars-old/opponents/OPP15_ciel_<YYYY-MM> archive/avatars-old/chapters`
   - `git mv public/avatars/opponents/OPP15_ciel/{character.png,background.png,icon.png,spec.md} archive/avatars-old/opponents/OPP15_ciel_<YYYY-MM>/`
   - `git mv public/avatars/chapters/chapter_15_ciel-landscape.png public/avatars/chapters/chapter_15_ciel-portrait.png archive/avatars-old/chapters/`
   - 旧 256px `archive/avatars-old/opponents/OPP15_ciel.png` は**触らない**
...
- [ ] 旧アセットが `archive/avatars-old/` に退避済み（live バンドルからは消えている）
- [ ] `grep -rn "シエル\|Ciel\|OPP15_ciel\|chapter_15_ciel" src/ public/avatars public/avatars/opponents/INDEX.md`
      が新コードに残っていない（`archive/avatars-old/`・`docs/scenario/` の歴史的記述は除外）
...
- **テキストだけ戻す**: `src/i18n/story/*.ts` の `chapterStories[14]` 等を該当 commit から revert。
  旧シエルテキストに戻る。
- **画像だけ戻す**: `git mv archive/avatars-old/opponents/OPP15_ciel_<YYYY-MM>/* public/avatars/opponents/OPP15_ciel/`
  ／章挿絵も `archive/avatars-old/chapters/` から戻す。`COMPUTERS` の `image` / `chapterArtBase` も旧
  `OPP15_ciel` パスに戻す。
